 ...
 * selectedItemId-branched rendering — see that file's "which selection UI
 * to show" comment) immediately before `stage.toDataURL()`, so the
 * exported PNG never shows selection/anchor chrome.
 *
 * The tricky part: `selectItem(null)` only queues a React state update.
 * `CanvasStage`'s selectedItemId branch re-renders asynchronously, and even
 ...
 * so a snapshot taken synchronously right after calling `selectItem(null)`
 ...
 *
 * `selectedItemId`/`selectItem` are passed in (rather than this function
 * reaching into `canvasStore` itself) so it stays a plain, Konva-adjacent
 * function callable/testable without a mounted store or component tree —
 * same "pure logic, thin Konva plumbing at the call site" split every other
 * canvas/*.ts helper in this codebase already follows (see
 * `LineAnchorHandles.test.tsx`'s doc comment on this convention).
 */
